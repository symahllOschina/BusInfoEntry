package com.wdkj.dkhdl.date.picker;
...
public class TimeSelector {
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            seletorDialog = new Dialog(context, R.style.PickerDialog);
...
//            int width = ScreenUtil.getInstance(context).getScreenWidth();
    ...
    private void initView() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                handler.handle(format(selectedCalender.getTime(), FORMAT_STR));
                seletorDialog.dismiss();
            }
        });
    ...
    private boolean excuteWorkTime() {
        boolean res = true;
        if (Utils.isNotEmpty(workStart_str) && !Utils.isNotEmpty(workEnd_str)) {
