...
public class Utils {
	...
	/**
	 * 判断执照证件号码是否正确（全国统一为两种格式：15位和18位）例如：92422801MA4BW3A356
	 * 验证15位：reg = '/^\d{15}$/';
	 * 验证18位：reg = '/^(?:(?![IOZSV])[\dA-Z]){2}\d{6}(?:(?![IOZSV])[\dA-Z]){10}$/';
	 * 合并兼容验证：reg = '/(^(?:(?![IOZSV])[\dA-Z]){2}\d{6}(?:(?![IOZSV])[\dA-Z]){10}$)|(^\d{15}$)/';
	 *
	 */
